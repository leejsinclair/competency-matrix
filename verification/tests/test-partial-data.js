...
(async () => {
  ...
  try {
    ...
    // Select a developer with partial data
    const developerSelect = await page.$('select');
    await developerSelect.selectOption({ label: 'azhelnin_creditsense' });
    await page.waitForTimeout(3000);
    
    console.log('=== PARTIAL DATA DESCRIPTIONS TEST ===');
    ...
    // Take screenshot
    await page.screenshot({ path: 'partial-data-test.png', fullPage: true });
    console.log('\n📸 Screenshot saved to partial-data-test.png');
