...
test.describe("Matrix Page - Comprehensive Button Testing", () => {
  ...
  test("all matrix page buttons - comprehensive testing", async ({ page }) => {
    ...
    const individualViewButton = page.locator(
    ...
    const compareDevelopersButton = page.locator(
      'button:has-text("Compare Developers")'
    );
    ...
    const clickableCells = page.locator(
      '.cursor-pointer, [data-testid="matrix-cell"], div[class*="cursor"]'
    );
    ...
    if (cellCount > 0) {
      ...
      for (let i = 0; i < cellsToTest; i++) {
        ...
        const modalClose = page.locator(
          'button:has-text("Close"), button:has-text("Cancel"), .bg-black'
        );
    ...
    const evidenceTriggerCells = page.locator(
      '[data-evidence="true"], .cursor-pointer'
    );
  ...
  test("matrix page - accessibility and keyboard navigation", async ({
    page,
  }) => {
    await page.goto("http://localhost:5173/matrix");
    await page.waitForLoadState("networkidle");
