...
import { ArtifactStore, ArtifactMetadata } from "../../src/types/artifact";
...
function createMockArtifactStore(): ArtifactStore {
  return {
    put: async (_key: string, _data: string, _metadata?: ArtifactMetadata) => {
    ...
    list: async () => {
      // Mock implementation - return empty list with correct structure
...
export class CompetencyAssessmentRunner {
  ...
  constructor() {
    this.artifactStore = createMockArtifactStore();
  ...
  async runSingleTest(
    ...
    const event = this.convertToActivityEvent(testContent);
    ...
    return {
      ...
      source:
        testContent.source === "slack"
          ? "git"
          : (testContent.source as ActivitySource),
...
export async function runAssessmentCommand(
  ...
  switch (type) {
    ...
    case "level":
      ...
      const levelResults: AssessmentResult[] = [];
