...
describe("Confluence Content Processor", () => {
  ...
  test("should process sample confluence content", async () => {
    ...
    const spaceDir = join(testDir, "ABT");
    if (!existsSync(spaceDir)) {
      mkdirSync(spaceDir, { recursive: true });
    }

    // Write sample page in space directory
    const pagePath = join(spaceDir, "sample-page.json");
    ...
    const processedPages = JSON.parse(
      require("fs").readFileSync(processedPagesPath, "utf8")
    );

    expect(processedPages).toHaveLength(1);
    expect(processedPages[0].original.title).toBe(
      "Java Microservices Architecture Guide"
    );

    // The processor applies all default rules, so expect more than 5 labels
    expect(processedPages[0].classification.labels.length).toBeGreaterThan(5);
    expect(processedPages[0].contributorInfo.email).toBe(
      "john.doe@company.com"
    );
    expect(processedPages[0].contributorInfo.displayName).toBe("john.doe");

    // Check specific classifications (these should be present among the many labels)
