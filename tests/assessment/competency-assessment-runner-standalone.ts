...
function createAsyncEchoMock<T extends (...args: any[]) => Promise<any>>(): T {
  return (async (...args: Parameters<T>) =>
    args as Awaited<ReturnType<T>>) as T;
}
...
export class CompetencyAssessmentRunner {
  ...
  constructor() {
    // Mock artifact store for assessment without Jest
    this.artifactStore = {
      put: createAsyncEchoMock<ArtifactStore["put"]>(),
      get: createAsyncEchoMock<ArtifactStore["get"]>(),
      delete: createAsyncEchoMock<ArtifactStore["delete"]>(),
      list: createAsyncEchoMock<ArtifactStore["list"]>(),
      exists: createAsyncEchoMock<ArtifactStore["exists"]>(),
      getMetadata: createAsyncEchoMock<ArtifactStore["getMetadata"]>(),
    };
